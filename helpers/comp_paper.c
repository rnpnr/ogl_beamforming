...
global WorkFrame work[] = {
	...
	{s8("250317_MN45-1_3.30MHz_ATS539_Cyst_uFORCES-32-TxRow"), low_freq_contrast_settings},
...
function void
fftw_create_filter(fftw_context *ctx, u32 samples, f32 sampling_frequency, f32 cutoff_frequency,
                   u32 order, b32 low_pass, b32 analytic)
{
	ctx->filter        = fftwf_alloc_complex(samples);
	ctx->filter_length = samples / 2;

	if (!ctx->filter)
		die("failed to allocate space for signal filter\n");

	if (low_pass) {
		f32 *f_in  = fftwf_alloc_real(samples);
		if (!f_in)
			die("failed to allocate space for temporary filter\n");

		fftwf_plan plan = fftwf_plan_dft_r2c_1d(samples, f_in, ctx->filter, FFTW_MEASURE);
		...
		fftwf_execute_dft_r2c(plan, f_in, ctx->filter);
		fftwf_free(f_in);
	...
	for (;;) {
		...
		if (fftw.filter_length) {
			...
			for (u32 transmit = 0; transmit < ctx.transmits; transmit++) {
				...
				for (u32 sample = 0; sample < fftw.filter_length; sample++) {
					u32 index = base_off + sample_off + sample;
					f32 x = fftw.fft[index][0];
					f32 y = fftw.fft[index][1];
					f32 a = fftw.filter[sample][0];
					f32 b = fftw.filter[sample][1];

					fftw.fft[index][0] = x * a - y * b;
					fftw.fft[index][1] = x * b + y * a;
				}
			}
		}
...
function b32
execute_work_item(WorkFrame *w, Arena arena, Stream path, Options *options, f32 *out_data,
                  u32 average_frames, u32 frame_off, b32 export)
{
	...
	b32 do_fft = options->analytic || options->low_pass;
	fftw_context *fftw = 0;
	if (do_fft) {
		...
		fftw_create_filter(fftw, bp.dec_data_dim.x, bp.sampling_frequency,
		                   w->settings.cutoff_frequency, w->settings.filter_order,
		                   options->low_pass, options->analytic);
	...
	/* TODO(rnp): this is innacurate for larger orders */
	if (options->low_pass) {
		bp.time_offset += (w->settings.filter_order - 1)
		                  / (4 * PI * w->settings.cutoff_frequency);
	}
...
work_loop(Options *options, s8 path_base)
{
	...
	do {
		for (i32 i = 0; i < ARRAY_COUNT(work); i++) {
			...
			b32 result = execute_work_item(w, arena, path, options, out_data,
