...
#define joy_res_settings (WorkGroupSettings){ \
	.output_path    = s8("beamformed"),              \
	.axial_extent   = {.x =  5e-3,  .y = 70e-3},     \
	.lateral_extent = {.x = -18.5e-3, .y = 18.5e-3}, \
	.frame_count    = 8,                             \
	.f_number       = 1,                             \
}
...
global WorkFrame work[] = {
	...
	//{s8("250417_A06_7.80MHz_ATS539_Resolution_FORCES-TxRow"), joy_res_settings},
...
execute_work_item(WorkContext *wc, WorkFrame *w, Arena arena, Stream path, s8 output_name)
{
	...
	if (raw_data_dim.z > 1) {
		...
		for (u32 i = 0; i < raw_data_dim.z; i++) {
			...
			send_frame(data + offset, fftw, 0, &bp);
