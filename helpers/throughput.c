...
b32 set_beamformer_parameters(char *shm_name, PARAMS_TYPE *);
b32 set_beamformer_pipeline(char *shm_name, i32 *stages, i32 stages_count);

b32 beamformer_start_compute(char *shm_name, u32 image_plane_tag);
b32 beamformer_push_data(char *shm_name, void *data, u32 data_size, i32 timeout_ms);
b32 beamformer_push_channel_mapping(char *shm_name, i16 *mapping, u32 count, i32 timeout_ms);
b32 beamformer_push_sparse_elements(char *shm_name, i16 *elements, u32 count, i32 timeout_ms);
b32 beamformer_push_focal_vectors(char *shm_name, f32 *vectors, u32 count, i32 timeout_ms);
b32 beamformer_push_parameters(char *shm_name, BeamformerParameters *bp, i32 timeout_ms);
b32 beamformer_push_parameters_ui(char *shm_name, BeamformerUIParameters *, i32 timeout_ms);
b32 beamformer_push_parameters_head(char *shm_name, BeamformerParametersHead *, i32 timeout_ms);
...
send_frame(i16 *restrict i16_data, PARAMS_TYPE *restrict bp)
{
	...
	#if NEW_PUSH_API
	if (beamformer_push_data(OS_SMEM_NAME, i16_data, data_size, 0))
		result = beamformer_start_compute(OS_SMEM_NAME, IPT_XZ);
	#else
	result = send_data(OS_PIPE_NAME, OS_SMEM_NAME, i16_data, data_size);
	#endif
...
execute_work_item(WorkFrame *w, Arena arena, Stream path, Options *options)
{
	...
	if (options->loop) {
		u32 frame = 0;
		f32 times[32] = {0};
		f32 data_size = bp.rf_raw_dim.x * bp.rf_raw_dim.y * sizeof(*data);
