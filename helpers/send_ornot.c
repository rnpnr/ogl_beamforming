...
static f32 g_width = 25e-3;
static f32 g_depth = 80e-3;
static i32 g_shader_stages[] = {
	CS_DECODE,
	//CS_DECODE_FLOAT,
	//CS_CUDA_DECODE,
	//CS_CUDA_HILBERT,
...
{
	if (argc < 2)
		die_(NULL, "usage: %s data_directory [file_number]\n", argv[0]);
	...
	size path_base_widx = path.widx;

	stream_append_s8(&path, s8("parameters_Intensity.bp"));
	stream_append_byte(&path, 0);

	zemp_bp_v1 *zbp = read_zemp_bp_v1(path.data);
	if (!zbp) die("failed to unpack parameters file\n");

	path.widx = path_base_widx;

	BeamformerParameters bp = {0};
	fill_beamformer_parameters_from_zemp_bp_v1(zbp, &bp);
	bp.output_points         = g_output_points;
	bp.output_min_coordinate = (v4){.x = -g_width, .z = 0};
	bp.output_max_coordinate = (v4){.x =  g_width, .z = g_depth};
	bp.f_number              = 1;

	set_beamformer_parameters(OS_SMEM_NAME, &bp);
	set_beamformer_pipeline(OS_SMEM_NAME, g_shader_stages, ARRAY_COUNT(g_shader_stages));
