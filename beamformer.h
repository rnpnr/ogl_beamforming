...
typedef struct { iptr handle; } LoadRFDataContext;
...
#define BEAMFORMER_WORK_QUEUE_PUSH_FN(name) BeamformWork *name(BeamformWorkQueue *q)
typedef BEAMFORMER_WORK_QUEUE_PUSH_FN(beamformer_work_queue_push_fn);
