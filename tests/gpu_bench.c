...
#define BSCAN_MIN      (v3){{-60e-3f,       0,  25e-3f}}
#define BSCAN_MAX      (v3){{ 60e-3f,       0, 130e-3f}}
#define VOLUME_MIN     (v3){{-30e-3f, -30e-3f,  30e-3f}}
#define VOLUME_MAX     (v3){{ 30e-3f,  30e-3f,  90e-3f}}
...
read_only global Work works[] = {
	...
	{.kind = WorkKind_Decode, .subdir = s8_comp("decode_256rx"), .decode = {.full_aperture = 1}},
	{
		...
		.compute = {
			...
			.output_points  = BSCAN_POINTS,
			.min_coordinate = BSCAN_MIN,
			.max_coordinate = BSCAN_MAX,
			.chirp_length   = 2e-5f,
	...
	},
	{
		...
		.subdir  = s8_comp("dtu_volume_linear"),
		.compute = {
			.study = s8_comp("251014_MN32-4_ATS_Cysts_2_VLS-128-Tx-Column-Chirp-2e-05"),
			.output_points   = DTU_VOLUME_POINTS,
			.min_coordinate  = DTU_VOLUME_MIN,
			.max_coordinate  = DTU_VOLUME_MAX,
			.interpolation   = BeamformerInterpolationMode_Linear,
	...
	},
	{
		...
		.compute = {
			...
			.output_points  = VOLUME_POINTS,
			.min_coordinate = VOLUME_MIN,
			.max_coordinate = VOLUME_MAX,
			.interpolation  = BeamformerInterpolationMode_Linear,
		},
	},
	{
...
beamformer_simple_parameters_from_file(u8 *file, BeamformerSimpleParameters *out)
{
	    ...
	    out->das_shader_id == BeamformerAcquisitionKind_UHERCULES)
	{
		...
		out->focal_vector[0] = zbp->transmit_angles[0];
		out->focal_vector[1] = zbp->focal_depths[0];
	} else {
...
function void
compute_work_parameters(BeamformerSimpleParameters *bsp, u8 *bp_filepath, ComputeWork *cw)
{
	beamformer_simple_parameters_from_file(bp_filepath, bsp);

	mem_copy(bsp->output_points, cw->output_points.E, sizeof(bsp->output_points));
	bsp->output_points[3] = 1;

	mem_copy(bsp->output_min_coordinate, cw->min_coordinate.E, sizeof(bsp->output_min_coordinate));
	mem_copy(bsp->output_max_coordinate, cw->max_coordinate.E, sizeof(bsp->output_max_coordinate));
	...
	u32  filter_parameters_count;
	f32 *filter_parameters;
	if (cw->chirp_length == 0) {
		...
		filter_parameters       = (f32 *)&filter.Kaiser;
		filter_parameters_count = sizeof(filter.Kaiser) / sizeof(f32);
	} else {
		filter.MatchedChirp.duration      = cw->chirp_length;
		filter.MatchedChirp.min_frequency = cw->bandwidth_min - bsp->demodulation_frequency;
		filter.MatchedChirp.max_frequency = cw->bandwidth_max - bsp->demodulation_frequency;

		filter_kind             = BeamformerFilterKind_MatchedChirp;
		filter_parameters       = (f32 *)&filter.MatchedChirp;
		filter_parameters_count = sizeof(filter.MatchedChirp) / sizeof(f32);
		filter_is_complex       = 1;

		bsp->time_offset += cw->chirp_length / 2;
	}

	beamformer_create_filter(filter_kind, filter_parameters, filter_parameters_count,
...
function void
decode_work_parameters(BeamformerSimpleParameters *bsp, u32 transmit_count, b32 full_aperture)
{
	...
	mem_copy(bsp->raw_data_dimensions, raw_data_dim(transmit_count).E, sizeof(bsp->raw_data_dimensions));
	mem_copy(bsp->channel_mapping, channel_mapping, sizeof(bsp->channel_mapping));

	bsp->compute_stages[0]    = BeamformerShaderKind_Decode;
...
execute_and_write(BeamformerSimpleParameters *bsp, u8 *path, i16 *data)
{
	...
	if (result) {
		...
		u32 data_size = bsp->raw_data_dimensions[0] * bsp->raw_data_dimensions[1] * sizeof(*data);
		...
		if (beamformer_compute_timings(&stats, 10000)) {
...
main(i32 argc, char *argv[])
{
	...
	if (!beamformer_set_global_timeout(10000)) {
		lib_error();
		die("");
	}
	...
	for EachElement(works, it) {
		...
		case WorkKind_Decode:{
			...
			for EachElement(decode_transmit_counts, transmits) {
				u32 transmit_count = decode_transmit_counts[transmits];
				printf("count: %u\n", transmit_count);
				...
				decode_work_parameters(bsp, transmit_count, work->decode.full_aperture);
		...
		case WorkKind_FullCompute:{
			...
			compute_work_parameters(bsp, input.data, &work->compute);
