...
execute_study_with_block(s8 study, Stream path, f32 chirp_length, BeamformerViewPlaneTag tag, u8 block)
{
	...
	if (zbp->beamform_mode == BeamformerAcquisitionKind_UHERCULES ||
	    zbp->beamform_mode == BeamformerAcquisitionKind_UFORCES)
	{
	    ...
	    bp.das_shader_id == BeamformerAcquisitionKind_UHERCULES)
	{
		...
		beamformer_push_transmit_receive_orientations_at(transmit_receive_orientations,
		                                                 countof(transmit_receive_orientations), block);
	}
