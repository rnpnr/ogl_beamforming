...
usage(char *argv0)
{
	die("%s parameters_file_test paremeter_file_interference\n", argv0);
}
...
function b32
send_frame(BeamformerSimpleParameters *restrict bp, void *restrict data, void *restrict output_data)
{
	u32 data_size = bp->raw_data_dimensions.E[0] * bp->raw_data_dimensions.E[1]
	                * beamformer_data_kind_byte_size[bp->data_kind];
	b32 result = beamformer_beamform_data(bp, data, data_size, output_data, 10000);
...
function void
execute_study(Arena *arena, Stream path, Options *options, void *restrict output_data)
{
	...
	send_frame(&bp, data, output_data);
}
...
entry_point(i32 argc, char *argv[])
{
	...
	i64 output_points = g_output_points.x * g_output_points.y * g_output_points.z;
	v2 *frame1 = push_array(arena, v2, output_points);
	v2 *frame2 = push_array(arena, v2, output_points);

	execute_study(arena, path, &options, frame1);

	stream_reset(&path, 0);
	stream_append_str8(&path, str8_from_c_str(options.remaining[1]));

	execute_study(arena, path, &options, 0);

	stream_reset(&path, 0);
	stream_append_str8(&path, str8_from_c_str(options.remaining[0]));

	execute_study(arena, path, &options, frame2);

	b32 result = memcmp(frame1, frame2, sizeof(v2) * output_points) == 0;
	printf("frames are%s byte wise identical\n", result ? "" : " NOT");
}
