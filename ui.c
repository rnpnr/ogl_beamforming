...
#define SELECTION_COLOUR       (v4){{0.07f, 0.37f, 0.90f, 1.0f}}
...
typedef enum {
	...
	UINodeFlag_TextInput                 = 1ull << 31,
	UINodeFlag_TextInputNumeric          = 1ull << 32,
...
{
	Font result = node->font_size < 28.0f ? ui_context->small_font : ui_context->font;
	return result;
}
...
{
	b32 result = 0;
	NumberConversion number = number_from_s8(s8_from_str8(s));
...
ui_text_input_cursor_range(void)
{
	...
	iv2 range;
	range.x = Min(tis->cursor_range.x, tis->cursor_range.y);
	range.y = Max(tis->cursor_range.x, tis->cursor_range.y);
	return range;
}
...
function void
ui_text_input_parts(str8 parts[3])
{
	UITextInputState *tis = &ui_context->text_input_state;
	iv2 range = ui_text_input_cursor_range();
	parts[0] = (str8){.data = tis->buffer,           .length = range.x};
	parts[1] = (str8){.data = tis->buffer + range.x, .length = range.y    - range.x};
	parts[2] = (str8){.data = tis->buffer + range.y, .length = tis->count - range.y};
}
...
ui_text_input_end(void)
{
	...
	tis->cursor_range.x = tis->cursor_range.y = 0;
...
ui_text_input_insert(str8 text)
{
	...
	tis->cursor_range.x = tis->cursor_range.y = cursor_range.x + truncated_length;
}

function b32
ui_text_input_update(void)
{
	UITextInputState *tis = &ui_context->text_input_state;

	// NOTE(rnp): handle multiple input keys on a single frame
	// TODO(rnp): this should just get a string from the os input
	for (i32 key = GetCharPressed();
	     tis->count < (i32)countof(tis->buffer) && key > 0;
	     key = GetCharPressed())
	{
		b32 allow_key = !tis->numeric || (Between(key, '0', '9') || (key == '.') ||
		                 (key == '-' && tis->cursor_range.x == 0));
		if (allow_key) {
			u8 byte = (u8)key;
			ui_text_input_insert(str8_struct(&byte));
		}
	}

	b32 left  = IsKeyPressed(KEY_LEFT)  || IsKeyPressedRepeat(KEY_LEFT);
	b32 right = IsKeyPressed(KEY_RIGHT) || IsKeyPressedRepeat(KEY_RIGHT);
	b32 shift = IsKeyDown(KEY_LEFT_SHIFT)   || IsKeyDown(KEY_RIGHT_SHIFT);
	// TODO(rnp): ctrl is a flag specifying word movement
	// TODO(rnp): shift is a flag specifying apply selection across movement
	//b32 ctrl  = IsKeyDown(KEY_LEFT_CONTROL) || IsKeyDown(KEY_RIGHT_CONTROL);

	tis->cursor_range.y += right;
	tis->cursor_range.y -= left;
	tis->cursor_range.y  = Clamp(tis->cursor_range.y, 0, tis->count);
	if ((left || right) && !shift)
		tis->cursor_range.x = tis->cursor_range.y;

	b32 delete    = IsKeyPressed(KEY_DELETE)    || IsKeyPressedRepeat(KEY_DELETE);
	b32 backspace = IsKeyPressed(KEY_BACKSPACE) || IsKeyPressedRepeat(KEY_BACKSPACE);
	if (delete || backspace) {
		if (tis->cursor_range.x == tis->cursor_range.y) {
			if (backspace) tis->cursor_range.x = Max(0, tis->cursor_range.x - 1);
			if (delete)    tis->cursor_range.y = Min(tis->count, tis->cursor_range.y + 1);
		}

		if (tis->cursor_range.x != tis->cursor_range.y)
			ui_text_input_insert(str8(""));
	}

	if (left || right || delete || backspace)
		tis->blinker.t = 1.0;

	// NOTE(rnp): check for text ending conditions
	b32 mouse_any_pressed = IsMouseButtonPressed(MOUSE_BUTTON_LEFT) ||
	                        IsMouseButtonPressed(MOUSE_BUTTON_RIGHT) ||
	                        IsMouseButtonPressed(MOUSE_BUTTON_MIDDLE);
	b32 result = IsKeyPressed(KEY_ENTER);
	result |= mouse_any_pressed && !ui_node_key_equal(ui_context->hot_node_key, tis->node_key);
	return result;
}
...
ui_signal_from_node(UINode *node)
{
	...
	b32 collides = !!point_in_rect(ui->current_mouse, nr);
	...
	// TODO(rnp): for (event in events)
	{
		...
		if ((node->flags & UINodeFlag_MouseClickable) && mouse_any_pressed && collides) {
			...
			ui->active_node_key[mouse_click] = node->key;
		...
		if ((node->flags & UINodeFlag_MouseClickable) &&
		     mouse_any_released &&
		     ui_node_key_equal(ui->active_node_key[mouse_release], node->key))
		{
			ui->hot_node_key                   = ui_node_key_zero();
			ui->active_node_key[mouse_release] = ui_node_key_zero();
	...
	if (node->flags & UINodeFlag_TextInput) {
		...
		if (result.flags & UISignalFlag_Pressed &&
		    !ui_node_key_equal(ui_context->text_input_state.node_key, node->key))
		{
			ui_context->text_input_state.next_node_key = node->key;
		}

		// NOTE(rnp): signal ended text input
		if (ui_node_key_equal(ui->text_input_state.last_node_key, node->key)) {
...
ui_tweak_f32_compute_variable(UISignal signal, f32 *value, f32 text_scale, f32 scroll_scale, v2 limits)
{
	if (signal.flags) {
		...
		if (!f32_equal(*value, (f32)new_value)) {
				                             ...
				                             str8("###parameter_listing_container")))
				{
					...
					UIPrefHeight(ui_text_dim(1.05f, 1.0f))
					{
						...
						UIFlags(UINodeFlag_Scroll|UINodeFlag_TextInputNumeric)
						{
							...
							ui_tweak_f32_compute_variable(signal, &bp->speed_of_sound, 1.f, 10.f, (v2){{0, F32_INFINITY}});
						}
						...
						UIFlags(UINodeFlag_Scroll|UINodeFlag_TextInputNumeric)
						{
							...
							ui_tweak_f32_compute_variable(signal, &ui->off_axis_position, 1e-3f, 0.1e-3f,
							                              (v2){{-F32_INFINITY, F32_INFINITY}});
						}
						...
						UIFlags(UINodeFlag_Scroll|UINodeFlag_TextInputNumeric)
						{
							...
							ui_tweak_f32_compute_variable(signal, &ui->beamform_plane, 1.f, 0.025f, (v2){{-1.f, 1.f}});
						}
						...
						UIFlags(UINodeFlag_Scroll|UINodeFlag_TextInputNumeric)
						{
							...
							ui_tweak_f32_compute_variable(signal, &bp->f_number, 1.f, 0.05f, (v2){{0, F32_INFINITY}});
						}
...
ui_draw_nodes(void)
{
	...
	while (stack.count) {
		...
		if (node->flags & UINodeFlag_DrawText) {
			...
			TextSpec text_spec = {.font = &font, .flags = TF_LIMITED, .colour = FG_COLOUR, .limits.size = r.size};

			if (ui_node_key_equal(node->key, ui->text_input_state.node_key)) {
				// TODO(rnp): don't do this here, maybe patch up the text node to use a custom draw path
				str8 parts[3];
				ui_text_input_parts(parts);

				v2 pos = r.pos;
				pos.x += measure_text(font, parts[0]).x;

				Rect cursor = {.pos = pos};
				if (parts[1].length > 0) {
					cursor.size = measure_text(font, parts[1]);
					DrawRectanglePro(rl_rect(cursor), (Vector2){0}, 0, colour_from_normalized(SELECTION_COLOUR));
				} else {
					cursor.size.x = parts[2].length > 0 ? 4.0f : 0.55f * (f32)font.baseSize;
					cursor.size.y = font.baseSize;
				}

				// TODO(rnp): cleanup: text input visible part
				draw_text(ui_text_input_string(), r.pos, &text_spec);
	...
	// NOTE(rnp): Text Input
	{
		...
		if (!ui_node_key_nil(tis->next_node_key)) {
			...
			if (point_in_rect(ui->current_mouse, ui_text_input_rect()))
				tis->cursor_range.x = tis->cursor_range.y = ui_text_input_index_from_point(ui->current_mouse.x);
			tis->blinker.t = 1.0f;
		}

		// NOTE(rnp): perform input handling
		if (!ui_node_key_nil(tis->node_key)) {
			...
			if (signal.flags & UISignalFlag_LeftDragging)
				tis->cursor_range.y = ui_text_input_index_from_point(ui->current_mouse.x);
