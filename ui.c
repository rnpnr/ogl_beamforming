	...
	if (var) {
		...
		while (var) {
			if (var->type == VT_GROUP) {
				...
				Variable *next = var->next, *parent = var->parent;
...
ui_export_view(BeamformerUI *ui, Variable *view)
{
	...
	stream_append_byte(&buf, '\n');
	if (!os->write_new_file((c8 *)path.data, stream_to_s8(&buf)))
		os->write_file(os->error_handle, s8("failed to export view parameters\n"));
	...
	if (out_buf) {
		...
		if (!os->write_new_file((c8 *)path.data, raw))
			os->write_file(os->error_handle, s8("failed to export view\n"));
		free(out_buf);
	}
}
