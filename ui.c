...
ui_export_view(BeamformerUI *ui, Variable *view)
{
	...
	stream_append_s8s(&sb, s8("."), os->path_separator);
	push_das_shader_kind(&sb, f->das_shader_kind, f->compound_count);
	...
	case BeamformerFrameViewKind_3DXPlane:{
		...
		stream_append_s8(&sb, s8("_X_Plane.bmp"));
	...
	default:{
		...
		stream_append_s8s(&sb, base, s8(".bin"));
		s8 data_file   = arena_stream_commit_and_reset(&scratch, &sb);
		...
		if (out_buf) {
			...
			if (!os_write_new_file((c8 *)data_file.data, raw))
				os_write_file(os->error_handle, s8("failed to export view\n"));
			free(out_buf);
		}
	}break;
