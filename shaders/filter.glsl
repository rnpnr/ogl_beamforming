...
{
	SAMPLE_TYPE result = SAMPLE_TYPE_CAST(input_data.values[index]);
	return result;
}
...
void main()
{
	...
	if (out_sample < SampleCount / DecimationRate) {
		...
		for (uint j = 0; j < FilterLength; j++)
			result += apply_filter(rf[offset + j], filter_coefficients.values[j]);
		output_data.values[out_offset] = RESULT_TYPE_CAST(result);
	}
}
