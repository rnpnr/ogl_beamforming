...
// TODO(rnp): make readonly/non-readonly variants
layout(std430, buffer_reference) restrict buffer F32 {
	f32 x[];
};

layout(std430, buffer_reference) restrict readonly buffer F16  { f16 x[];     };
layout(std430, buffer_reference) restrict readonly buffer F32C { f32vec2 x[]; };
layout(std430, buffer_reference) restrict readonly buffer U32  { u32 x[];     };
...
void median_2d_3x3()
{
	...
	for (s32 y = -1; y <= 1; ++y) {
		for (s32 x = -1; x <= 1; ++x) {
			...
			{
				v[index][0] = F32(HeapBase + InputBuffer).x[3 * voxel_index + 0];
				v[index][1] = F32(HeapBase + InputBuffer).x[3 * voxel_index + 1];
				v[index][2] = F32(HeapBase + InputBuffer).x[3 * voxel_index + 2];
				index++;
			} else {
				v[index++] = vec3(0.f);
			}
	...
	F32(HeapBase + OutputBuffer).x[3 * out_index + 0] = v[4][0];
	F32(HeapBase + OutputBuffer).x[3 * out_index + 1] = v[4][1];
	F32(HeapBase + OutputBuffer).x[3 * out_index + 2] = v[4][2];
}
