...
layout(std430, buffer_reference, buffer_reference_align = 64) restrict readonly buffer RFBuffer {
	INPUT_DATA_TYPE values[];
};

layout(std430, buffer_reference, buffer_reference_align = 64) restrict writeonly buffer OutputRFBuffer {
	INPUT_DATA_TYPE values[];
};

layout(std430, buffer_reference, buffer_reference_align = 64) restrict writeonly buffer OutputBuffer {
	SAMPLE_DATA_TYPE values[];
};

layout(std430, buffer_reference, buffer_reference_align = 64) restrict readonly buffer HadamardBuffer {
	float values[];
};

SAMPLE_DATA_TYPE sample_rf_data(uint index)
{
	SAMPLE_DATA_TYPE result = SAMPLE_DATA_TYPE(rf_buffer.values[index]);
	return result;
}
...
void run_decode_large(void)
{
	...
	for (uint i = 0; i < samples_this_thread; i++) {
		...
		rf[index] = rf_buffer.values[rf_offset + index];
	}
	...
	if (time_sample < OutputTransmitStride) {
		...
		for (int j = 0; j < TransmitCount; j++) {
			...
			for (uint i = 0; i < ToProcess; i++)
				result[i] += s * hadamard_buffer.values[TransmitCount * j + (i + transmit)];
		}
	...
	if (time_sample < OutputTransmitStride) {
		...
		for (uint i = 0; i < ToProcess; i++, out_off += OutputTransmitStride)
			if (TransmitCount % (gl_WorkGroupSize.z * ToProcess) == 0 || transmit + i < TransmitCount)
				output_buffer.values[out_off] = result[i];
	}
}
#endif
...
void run_decode_small(void)
{
	...
	if (time_sample < OutputTransmitStride) {
		...
		for (int j = 0; j < TransmitCount; j++)
			rf[j] = rf_buffer.values[rf_offset + j];
		...
		for (int i = 0; i < TransmitCount; i++) {
			...
			for (int j = 0; j < TransmitCount; j++) {
				result[j] += s * hadamard_buffer.values[TransmitCount * i + j];
			}
		}
...
void main()
{
	...
	case DecodeMode_None:{
		...
		if (time_sample < OutputTransmitStride) {
			...
			output_buffer.values[out_off] = sample_rf_data(in_off);
		}
	}break;
				...
				uint in_off  = InputChannelStride * channel + InputSampleStride * time_sample;
					...
					for (uint i = 0; i < ToProcess; i++, in_off += InputTransmitStride) {
						if (transmit + i < TransmitCount)
							output_rf_buffer.values[out_off + i] = rf_buffer.values[in_off];
					}
				#else
