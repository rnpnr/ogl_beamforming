...
layout(std430, buffer_reference, buffer_reference_align = 64) restrict readonly buffer RFData {
	SAMPLE_TYPE values[];
};
...
SAMPLE_TYPE cubic(const int base_index, const float index)
{
	...
	SAMPLE_TYPE samples[4] = {
		rf_data.values[base_index + int(tk) - 1],
		rf_data.values[base_index + int(tk) + 0],
		rf_data.values[base_index + int(tk) + 1],
		rf_data.values[base_index + int(tk) + 2],
	};
...
SAMPLE_TYPE sample_rf(const int channel, const int transmit, const float index)
{
	...
	case InterpolationMode_Nearest:{
		if (index >= 0 && int(round(index)) < SampleCount)
			result = rotate_iq(rf_data.values[base_index + int(round(index))], index / SamplingFrequency);
	}break;
	case InterpolationMode_Linear:{
		if (index >= 0 && round(index) < SampleCount) {
			float tk, t = modf(index, tk);
			int n = base_index + int(tk);
			result = (1 - t) * rf_data.values[n] + t * rf_data.values[n + 1];
		}
...
uint8_t tx_rx_orientation_for_acquisition(const int16_t acquisition)
{
	uint8_t result = bool(SingleOrientation) ? uint8_t(TransmitReceiveOrientation) : transmit_receive_orientations.values[acquisition];
	return result;
}

vec2 focal_vector_for_acquisition(const int16_t acquisition)
{
	vec2 result = bool(SingleFocus) ? vec2(TransmitAngle, FocusDepth) : focal_vectors.values[acquisition];
	return result;
}

float rca_transmit_distance(const vec3 world_point, const vec2 focal_vector, const uint8_t transmit_receive_orientation)
{
	float result = 0;
...
RESULT_TYPE RCA(const vec3 world_point)
{
	...
	for (int16_t acquisition = acquisition_start; acquisition < acquisition_end; acquisition++) {
		const uint8_t tx_rx_orientation = tx_rx_orientation_for_acquisition(acquisition);
...
{
	const uint8_t tx_rx_orientation = tx_rx_orientation_for_acquisition(int16_t(0));
	...
	for (int16_t transmit = int16_t(Sparse); transmit < int16_t(AcquisitionCount); transmit++) {
		const int16_t tx_channel = bool(Sparse) ? sparse_elements.values[transmit - Sparse] : transmit;
...
RESULT_TYPE FORCES(const vec3 world_point)
{
	...
	for (int16_t rx_channel = rx_channel_start; rx_channel < rx_channel_end; rx_channel++) {
		...
		if (a_arg < 0.5f) {
			float apodization = apodize(a_arg);
			for (int16_t transmit = int16_t(Sparse); transmit < int16_t(AcquisitionCount); transmit++) {
				int16_t tx_channel      = bool(Sparse) ? sparse_elements.values[transmit - Sparse] : transmit;
...
void main()
{
	...
	#if CoherencyWeighting
	  incoherent_output.values[out_index] += RESULT_INCOHERENT_CAST(sum);
	#endif

	output_data.values[out_index] += RESULT_COHERENT_CAST(sum);
}
