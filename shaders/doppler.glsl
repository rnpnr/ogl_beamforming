...
layout(std430, buffer_reference) readonly buffer Input {
	InputDataType x[];
};
...
void main()
{
	...
	for (u32 i = 0; i < n_per_thread; i++) {
		Input image = Input(ImageBuffer + U32(HeapBase + ImageOffsets).x[i]);
	...
	vec2 r1   = vec2(0.f);
	f32  r0   = 0.f;
	F16  f    = F16(HeapBase + FilterMatrix);
	...
	if (index < OutputVoxels) {
		F32 outbuf   = F32(HeapBase + OutputBuffer);
		F32 feedback = F32(HeapBase + FeedbackBuffer);
